...
## D29. 去AI味三层组合拳：生成时负面清单 + 划词/批量去味 pass + 词表扫描定量（源自 2026-08 调研）

- 调研存档 `docs/research-deai-2026-08.md`：AI 味本质是模板语言（一级禁用词/指纹句式/叙事级指纹），
  社区共识解法是三层组合（预防 + 改写 + 扫描），单靠一句"去AI味"提示词无效（原 SYSTEM_PROMPT 的教训）；
  核心参照 oh-story-claudecode（3.2k★）story-deslop 六门禁与禁用词表
- 层1 预防：SYSTEM_PROMPT / TRANSFORM / ASSISTANT_REWRITE 注入【去AI味】硬约束段
  （禁用词表 + 指纹句式禁令 + 情绪外化 + 章末禁升华）——所有正文生成路径一次覆盖，零额外成本
- 层2 改写：划词浮动条新增「去AI味」（`ai_transform mode=deslop`，六门禁压缩成一份
  `DESLOP_REQUIREMENT`，流式替换）；批量写章可选二遍 pass（设置 `batch_deslop`，
  每章生成后自跑 `deslop_chapter_text`，失败回退原文不中断，成本 ×2 由用户显式开启）
- 层3 定量：`scan_banned_words` 返回值扩为 `ScanResult{hits,total_words}`，体检页出
  命中密度（处/万字）+ 高频词 Top8；内置 AI 味词表一键填入（`src/lib/aiWords.ts`），
  命中章节可一键入队跨章去味改写（复用快照回滚链路）
- 未做：外部 AI 检测（朱雀等）无稳定 API 不接入；风格卡人味锚点（A5）与替换范例库（A6）留待迭代

## D30. 写作记忆三件套：台账回流 + 活设定 + 远期梗概分层（v21）

背景：核对发现写作注入链有「半断链」——大纲/设定/摘要每章都注入，但设定变更台账只提取不回流
（AI 不知道"第 N-5 章主角左臂已断"），且摘要预算 1500 字只装得下最近 10~15 章，远期剧情失忆。

- **台账回流（零成本）**：`build_changes_section` 取 next_order 之前最近几章的台账行
  （预算 500 字，时间正序），以【设定变更】段注入续写/批量写章 system prompt——设定库是开书快照，
  这段告诉 AI 剧情已改了什么；meta 明细加"近期设定变更 N 条"
- **活设定（人工触发，防 AI 漂移）**：`apply_lore_changes` 把待应用变更按章节顺序合并进设定库
  （new 建词条 / update 追加[剧情更新·第N章]并复活停用条目 / retire 停用保留内容可回溯）；
  applied_at 标记已应用，重复提取同章会重置为未应用；台账页一键应用。不做全自动：AI 自己改设定库
  会越改越歪，人审一遍是安全阀
- **远期梗概（分层记忆）**：era_summaries 表，`compress_era_summaries`（体检页入口）把最近
  15 章之前的摘要每 50 章压成 ≤200 字梗概（只留主线/人物重大变化/物品地图/未回收伏笔），
  增量压缩（已有覆盖段跳过，范围长大重算替换）；`build_memory_section` 注入时远期梗概（预算
  600 字，超了保最新）+ 近期摘要 verbatim（1500 字）两段拼装
- 顺带修：大纲注入从"只有节点标题"补上当前卷的 content（阶段目标/主要冲突/末局面变化，80 字）
- 未做：era 梗概自动过期（章节改写/重写后旧梗概不刷新，需重跑压缩覆盖）——跨章改写频率低，先记坑

## D31. 设定系统 LLM Wiki 化：注入时融合 + 三段提取 + 关系网络 + 重写式活设定（v22）

方案讨论存档见会话（用户拍板）：词条=状态页、台账=revision 流的底座不动，融合发生在注入与应用两层；
agentic wiki 检索经测算不省 token 不采用；向量检索记为后续候选（触发：词条过千或明显设定失忆）。

- **W1 注入时融合（零额外调用）**：`build_lore_section` 命中词条时连带该词条全量变更时间线
  （[剧情状态] 段，标注"当前状态以此为准"），取代 D30 的全局近期 500 字台账通道——
  老变化不再受全局窗口限制，词条内容不被流水账污染。护栏：单词条最多最近 8 条、合计 ≤280 字
  （超出的更早变更省略，远期梗概兜底）。生效范围：续写/批量写章/划词/助手/视频口播与分镜
- **W2 三段提取（+0 次调用）**：`extract_chapter_lore_changes` 单次调用输出
  changes（持久变更）+ entities（**穷尽收集融入自动链路**：所有新登场元素含次要物品，
  自动建词条 keywords=标题非常驻）+ relations（关系三元组：拥有/失去/使用/师承/敌对/结盟/属于/居住/创建，
  存 lore_relations，整章替换幂等）。宽容解析兼容旧数组格式
- **W3 关系网络**：详情面板「拥有/关联」读关系表（正向：人物→物品功法；反向：物品→持有者），
  无关系数据的旧书退回字符串匹配
- **W4 重写式活设定**：`apply_lore_changes` 升级——update 类变更由 LLM 把词条重写成
  "当前状态"叙述（原始卡+变更序列→≤300 字现在时描述，批量一次调用；LLM 失败回退机械追加）；
  重写前快照（lore_backups），`rollback_lore_apply` 按 ts 恢复快照 + 变更重置未应用；
  批量写章收尾自动跑（设置 auto_apply_lore 默认开，弹层可关）
- 风险留痕：同秒内两次 apply 共用 ts（回滚会一起回，可接受）；跨章改写后旧关系/实体不自动失效
  （重提取该章幂等覆盖 changes/relations，实体词条留待人工清理）
